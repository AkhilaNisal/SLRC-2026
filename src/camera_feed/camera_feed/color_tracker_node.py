...
class ColorTrackerNode(Node):
    def __init__(self):
        ...
        self.lower_blue = np.array([100, 120, 70], dtype=np.uint8)
        ...
        self.min_linear_speed = 0.8
        self.min_angular_speed = 0.5

        self.forward_speed_gain = 8.0
        ...
        # Deadbands
        self.center_deadband = 0.08
        self.area_deadband = 0.01
        ...
        self.get_logger().info('Blue tracker node started.')
    ...
    def apply_min_speed(self, value: float, min_speed: float, max_speed: float) -> float:
        if abs(value) < 1e-6:
            return 0.0
    ...
    def image_callback(self, msg: Image):
        ...
        if contours:
            ...
            if area_ratio >= self.min_area_ratio:
                ...
                if moments['m00'] > 0:
                    ...
                    # Horizontal error: positive if object is left of center
                    ...
                    if abs(x_error) < self.center_deadband:
                        ...
                        angular_z = self.apply_min_speed(
                            angular_z,
                    ...
                    if abs(area_error) < max(self.area_tolerance, self.area_deadband):
                        linear_x = 0.0
                    else:
                        linear_x = self.forward_speed_gain * area_error
                        linear_x = self.apply_min_speed(
                            linear_x,
                    ...
                    if self.show_debug:
                        ...
                        cv2.circle(frame, (cx, int(moments['m01'] / moments['m00'])), 6, (255, 0, 0), -1)
                        cv2.line(
                            frame,
                        ...
                        debug_text_1 = f'area_ratio={area_ratio:.4f} x_error={x_error:.3f}'
                        debug_text_2 = f'linear_x={linear_x:.3f} angular_z={angular_z:.3f}'
                        cv2.putText(
                            frame, debug_text_1, (10, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2
                        )
                        cv2.putText(
                            frame, debug_text_2, (10, 60),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2
                        )
        ...
        if self.show_debug:
            ...
            cv2.imshow('blue_tracker_debug', frame)
            cv2.waitKey(1)
