...
class RestoreTestClient(Node):
    def __init__(self):
        super().__init__("restore_test_client")
        ...
        self.get_logger().info(
            f"Restore test client started. Waiting for action server: {self.arm_action_name}"
        )

        self.start_restore_test()

    def start_restore_test(self):
        ...
        self.get_logger().info("Sending RESTORE goal...")
    ...
    def goal_response_cb(self, future):
        ...
        if not goal_handle.accepted:
            self.get_logger().error("RESTORE goal rejected by server.")
            rclpy.shutdown()
            return
    ...
    def result_cb(self, future):
        ...
        self.get_logger().info(
            f"RESTORE result: success={result.success}, message='{result.message}'"
        )
