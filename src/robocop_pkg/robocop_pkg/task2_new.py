...
class WhiteLineFollowerWithBoxVisit(Node):
    def __init__(self):
        ...
        self.declare_parameter('box_wait_time', 2.0)
        ...
        self.STATE_BOX_WAIT_AT_BOX = 'BOX_WAIT_AT_BOX'
        ...
        self.line_cross_turn_direction = +1.0   # +1 left, -1 right
        ...
        self.configure_line_cross_sequence(
            ...
            turn_direction=+1.0,     # initial left turn
            next_state=self.STATE_FOLLOW_LINE
        )

        self.get_logger().info("Started task2_new with reusable line-cross sequence.")
    ...
    def image_cb(self, msg: Image):
        ...
        if self.state in {
            ...
            if red_found:
                ...
                if self.box_stop_counter >= self.box_stop_frames:
                    self.state = self.STATE_BOX_WAIT_AT_BOX
                    self.box_wait_start_time = self.get_clock().now()
                    self.get_logger().info(
                        f"Reached {self.active_box_side} red box. "
                        f"range={self.fmt_range(side_dist)} area={int(red_area)}. Waiting."
                    )
            else:
        ...
        elif self.state == self.STATE_BOX_WAIT_AT_BOX:
            twist.linear.x = 0.0
            twist.angular.z = 0.0

            elapsed = (self.get_clock().now() - self.box_wait_start_time).nanoseconds / 1e9
            if elapsed >= self.box_wait_time:
                self.state = self.STATE_BOX_TURN_BACK_180
                self.turn_start_time = self.get_clock().now()
                self.get_logger().info("Box wait complete. Turning back 180 degrees.")
            ...
            # IMPORTANT FIX:
            # LEFT box  -> turn LEFT to resume
            # RIGHT box -> turn RIGHT to resume
        ...
        cv2.putText(
            vis,
            f"L={self.display_range(self.left_range)} R={self.display_range(self.right_range)}",
            (10, 95),
        ...
        cv2.putText(
            vis,
            f"Lraw={self.fmt_range(self.left_range_raw)} Rraw={self.fmt_range(self.right_range_raw)}",
            (10, 125),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (180, 180, 255),
        ...
        if (now - self.last_log_time).nanoseconds > 1_000_000_000:
            ...
            self.get_logger().info(
                ...
                f"left={self.display_range(self.left_range)} right={self.display_range(self.right_range)} "
